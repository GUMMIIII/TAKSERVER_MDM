...
mkdir -p \
    "$KOMMS_DIR/nginx/certs" \
    "$KOMMS_DIR/tak-release" \
    "$KOMMS_DIR/server/takserver" \
...
CERT_UNIT="${CERT_UNIT}"
EOF
sed -i 's/\r//' "$KOMMS_DIR/server/.env"
chmod 600 "$KOMMS_DIR/server/.env"
...
echo -e "  ${CYAN}${KOMMS_DIR}/users/${OPERATOR_USER}/${OPERATOR_USER}.ovpn${NC}"
echo ""
echo -e "  Copy it to your local machine:"
echo -e "  ${CYAN}scp root@$(curl -sf4 https://ifconfig.me 2>/dev/null || hostname -I | awk '{print $1}'):${KOMMS_DIR}/users/${OPERATOR_USER}/${OPERATOR_USER}.ovpn .${NC}"
echo ""
