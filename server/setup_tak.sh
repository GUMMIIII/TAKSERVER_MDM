...
echo -e "  WebTAK:            ${CYAN}https://tak.${DOMAIN}/${NC}            (Authelia login, lldap_admin group)"
echo -e "  Marti Dashboard:   ${CYAN}https://tak.${DOMAIN}/Marti/${NC}      (same SSO — no browser cert needed)"
echo -e "  ATAK client port:  ${CYAN}${DOMAIN}:8089${NC}                    (TLS, x509 client cert)"
echo -e "  Cert enrollment:   ${CYAN}https://${DOMAIN}:8444${NC}            (mTLS)"
echo ""
echo -e "  ${YELLOW}Notes:${NC}"
echo -e "  • nginx presents the admin client cert to TAKServer:8443 — both WebTAK and"
echo -e "    Marti are reachable on https://tak.${DOMAIN}/ with a single Authelia login."
echo -e "  • Issue user TAK certs via ${CYAN}add_user.sh <username>${NC} — generates a .p12 + ATAK data package."
echo ""
