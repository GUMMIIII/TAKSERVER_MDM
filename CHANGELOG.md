...
- **Jitsi Meet** — self-hosted encrypted video conferencing behind VPN + Authelia SSO.
  - Four-container stack: `jitsi-web`, `jitsi-prosody` (XMPP), `jitsi-jicofo` (conference focus), `jitsi-jvb` (video bridge).
  - Reachable at `https://meet.DOMAIN` — VPN required, Authelia gate (any authenticated user).
  - Port `10000/UDP` (JVB WebRTC media) is publicly exposed — WebRTC requires direct UDP for media; web UI and XMPP signaling remain VPN+Authelia-gated, making the raw UDP port useless without valid signaling.
  - No separate Jitsi accounts needed: Authelia SSO is the only gate; once through, rooms can be created and joined freely.
  - Secrets (`JICOFO_COMPONENT_SECRET`, `JICOFO_AUTH_PASS`, `JVB_AUTH_PASS`) and `JVB_ADVERTISE_IP` (public server IP) are auto-generated by `setup_server.sh` on first run.
  - `meet.DOMAIN` added to Let's Encrypt cert, UFW, nginx, Authelia access control, and both WORKFLOW guides.

### Fixed

- **`setup_server.sh` — Jitsi secrets not written on fresh install** — `sed` exits 0 even when no substitution is made, so the `||` fallback never triggered and all three Jitsi secrets (`JICOFO_COMPONENT_SECRET`, `JICOFO_AUTH_PASS`, `JVB_AUTH_PASS`) remained empty, causing containers to start misconfigured. Replaced with `_write_secret()` helper that explicitly checks key presence before deciding between sed-replace and append.
- **`setup_server.sh` — cert not expanded for `meet.DOMAIN`** — the TLS block only checked `if [[ ! -f fullchain.pem ]]` and skipped entirely on re-runs. New subdomains added after initial install were never added to the cert SAN. Fixed by parsing the current SAN list with `openssl x509` and running `certbot --expand` (with nginx stopped) whenever any required domain is missing.
- **Docs — `meet.DOMAIN` missing from DNS setup tables** in both `WORKFLOW.md` and `WORKFLOW.de.md`. Added the record row so new operators know to create the DNS entry before running the installer.
- **Docs — `.env.example` subdomain comment** did not list `meet.` alongside the other auto-derived subdomains.
